...
# Pre-compiled regex patterns for better performance in string conversions
UNSAFE_TO_SAFE_REGEX = Regexp.union(UNSAFE_TO_SAFE_STRING_MAPPINGS.keys).freeze
SAFE_TO_UNSAFE_REGEX = Regexp.union(SAFE_TO_UNSAFE_STRING_MAPPINGS.keys).freeze
...
def convert_from_real_to_query_safe(string)
  # Use cached regex for better performance - avoids regex compilation overhead
  string.to_s.gsub(UNSAFE_TO_SAFE_REGEX, UNSAFE_TO_SAFE_STRING_MAPPINGS)
end

def convert_from_query_safe_to_real(string)
  # Use cached regex for better performance - avoids regex compilation overhead
  string.to_s.gsub(SAFE_TO_UNSAFE_REGEX, SAFE_TO_UNSAFE_STRING_MAPPINGS)
end
